...
function redraw_oled () {
    ...
    OLED.writeString("BPM: ")
    // OLED 첫 번째 줄에 "BPM: "을 표시합니다.
    OLED.writeNumNewLine(bpm)
...
let THRESHOLD_HIGH = 600
let THRESHOLD_LOW = 550
...
basic.forever(function () {
    ...
    sensor_value = pins.analogReadPin(AnalogReadWritePin.P1)
    // 프로그램이 시작된 후 지금까지 지난 시간을 가져옵니다.
    now = input.runningTime()
    // 8-2. 새로운 박동 감지
    // 센서값이 높은 기준값을 넘었고,새로운 박동을 감지할 준비가 되어 있는지 확인합니다.
    if (sensor_value > THRESHOLD_HIGH && beat_armed) {
...
// micro:bit LED에 큰 하트를 표시합니다.
        ...
        if (last_beat > 0) {
            ...
            if (beat_interval >= MIN_INTERVAL && beat_interval <= MAX_INTERVAL) {
                // BPM 계산식
                // 1분은 60,000밀리초입니다.
...
heart_off_time = 0
    ...
    graph_y = Math.round(Math.map(limited_value, SIGNAL_MIN, SIGNAL_MAX, GRAPH_BOTTOM, GRAPH_TOP))
